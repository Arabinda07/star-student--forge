...
import { School, ArrowRight, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function SignIn({ onSuccess, onNavigateToSignUp, onBackToOnboarding }: SignInProps) {
  ...
  // Read email from local storage or URL if coming from signup
  useEffect(() => {
    const passedEmail = sessionStorage.getItem("signup_email");
    if (passedEmail) {
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-stone-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-stone-600 dark:text-stone-400">
          Or{" "}
          <button onClick={onNavigateToSignUp} className="font-bold text-[var(--color-brand-600)] dark:text-[var(--color-brand-400)] hover:underline focus:outline-none">
          ...
          <form className="space-y-6" onSubmit={handleSignIn}>
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full h-12 flex justify-center items-center gap-2 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white bg-stone-900 hover:bg-stone-800 dark:bg-stone-50 dark:text-stone-900 dark:hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-stone-900 disabled:opacity-50 transition-all"
              >
