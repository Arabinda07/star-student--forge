        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 -mx-2 px-2">
          {WEEK_DATA.map((day, i) => {
            ...
            return (
              ...
              >
                <span className={`text-[10px] font-bold uppercase tracking-widest font-sans mb-1.5 ${isSel ? "text-sky-100" : ""}`}>
                  {getDayName(i)}
                </span>
                ...
                <div className="flex gap-1 mt-2 h-1.5">
                  {Array.from({ length: Math.min(day.events, 3) }).map((_, j) => (
                    <div key={j} className={`w-1.5 h-1.5 rounded-full ${isSel ? "bg-sky-200" : "bg-stone-300"}`} />
                  ))}
                </div>
              </button>
            );
