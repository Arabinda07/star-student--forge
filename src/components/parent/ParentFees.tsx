        ...
        <div className="max-w-7xl mx-auto w-full relative z-10">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6 mb-2">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl md:text-5xl font-black text-stone-900 dark:text-stone-50 tracking-tighter font-sans leading-none mb-2">Fees & Billing</h1>
                <p className="text-[10px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-widest leading-none drop-shadow-sm">Rohan Das · Class 8 · 2025-26 Session</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-6 py-10 pb-24 space-y-10 mt-8">
        {due && !confirmed && (
          <motion.div 
            ...
            className="rounded-[40px] p-10 md:p-12 bg-white dark:bg-stone-900 border border-stone-200/60 dark:border-stone-800/60 shadow-sm relative overflow-hidden"
          >
            <div className="absolute top-0 left-0 right-0 h-2 bg-amber-400" />
            
            <div className="flex items-center gap-3 mb-8 bg-amber-50 dark:bg-amber-950/30 w-max px-3 py-1.5 rounded-[12px] border border-amber-200/60 dark:border-amber-900/40">
               <Clock size={16} strokeWidth={2.5} className="text-amber-500 font-black" />
               <span className="text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-500 font-sans">
                  Action Required: Payment Due
               </span>
            </div>

            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10 pb-10 border-b border-stone-200/60 dark:border-stone-800/60">
              <div>
                <div className="text-sm font-bold text-stone-500 dark:text-stone-400 uppercase tracking-widest mb-2 font-sans">{due.month} Installment</div>
                <div className="text-5xl md:text-7xl font-black font-sans text-stone-900 dark:text-stone-50 tracking-tight leading-none"> ₹{due.amount.toLocaleString("en-IN")} </div>
                <div className="flex items-center gap-2 mt-4 text-[11px] font-black uppercase tracking-widest text-stone-500 dark:text-stone-400 px-3 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-500/10 border border-rose-200/60 dark:border-rose-900/40 w-max"> 
                   <AlertCircle size={14} className="text-rose-500" strokeWidth={2.5}/>
                   <span className="text-rose-600 dark:text-rose-400">Deadline: {due.dueDate}</span>
                </div>
              </div>
              <div className="bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-amber-200/60 dark:border-amber-800/60 shadow-sm whitespace-nowrap">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-stone-900 border border-stone-200/60 dark:border-stone-800/60 rounded-[40px] shadow-sm flex flex-col p-16 text-center items-center justify-center min-h-[340px] relative overflow-hidden group">
             <div className="absolute inset-0 bg-stone-50/50 dark:bg-stone-950/20 opacity-0 group-hover:opacity-100 transition-opacity" />
             <div className="w-24 h-24 bg-stone-50 dark:bg-stone-800/50 rounded-[32px] flex items-center justify-center mx-auto mb-8 shadow-inner relative z-10 transition-transform duration-700 group-hover:scale-105 border border-stone-100 dark:border-stone-700/50">
                <History size={40} className="text-stone-400 dark:text-stone-500" strokeWidth={2} />
             </div>
             <div className="relative z-10">
                <p className="text-2xl md:text-3xl font-black text-stone-900 dark:text-stone-50 font-sans tracking-tight mb-3">Clean Slate</p>
                <p className="text-base font-bold text-stone-500 dark:text-stone-400 font-sans max-w-[280px] mx-auto leading-relaxed">Once you make a payment, your verified receipts will be archived here.</p>
             </div>
          </div>
        </div>
